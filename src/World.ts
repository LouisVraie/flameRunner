...
class World{
    ...
    async loadWorld(){
        ...
        //
        let pathGroup = new Mesh("pathGroup");

        let v3 = (x, y, z) => new Vector3(x,y,z);
        let curve = Curve3.CreateCubicBezier(v3(-10,10,0), v3(-5, 10, 10), v3(5, 10, 10), v3(10, 10, 0),40);
        let curveCont = Curve3.CreateCubicBezier(v3(10, 10, 0), v3(5, 10, -10), v3(-5, 10, -10), v3(-10, 10, 0), 40);
        curve = curve.continue(curveCont);
        var curveMesh = MeshBuilder.CreateLines(
        ...
        for(var p = 0; p < curvePath.length; p++) {
            var tg = MeshBuilder.CreateLines('tg', {points: [ curvePath[p], curvePath[p].add(tangents[p]) ]}, this._scene);
            //tg.color = ;
            tg.isVisible = false;
            tg.parent = pathGroup;
            var no = MeshBuilder.CreateLines('no', {points: [ curvePath[p], curvePath[p].add(normals[p]) ]}, this._scene);
            //no.color = null;
            no.isVisible = false;
            no.parent = pathGroup;
            var bi = MeshBuilder.CreateLines('bi', {points: [ curvePath[p], curvePath[p].add(binormals[p]) ]}, this._scene);
            ...
            this._checkpointTab.forEach((checkPoint, index) => {
                player.getCharacter().getHitbox().actionManager.registerAction(new ExecuteCodeAction(
                    ...
                    () => {
                        ...
                        let respawn = this._respawnTab.at(index).absolutePosition;
            ...
            this._deathTab.forEach((death, index) => {
                ...
                hitbox.actionManager.registerAction(new ExecuteCodeAction(
                    ...
                    () => {
                        ...
                        let respawnPosition = player.getCharacter().getSpawnLocation().clone();
